...
GetPrintArray(newArray, column, row);
...
// Метод переворота массива на 90 градусов: замены строк на столбцы в 2-мерном массиве 
int[,] GetNewArray(int[,] array, int row, int column)
{
    ...
    for (int i = 0; i < row; i++)
    {
        ...
        for (int j = 0; j < column; j++)
        {
            tempColumn[j]=array[i,j];
            newArray[j,i] = tempColumn[j];
        }
    }
    return newArray;
}